...
function Controller(client, security, options) {
    ...
    // add security to controller context
...
Controller.prototype.zero = function () {
    this.disable();
    this._security.reset(this.state());
    this._ekf.reset();
};
...
Controller.prototype.backward = function (distance, callback) {
    this.forward(-distance, callback);
};
...
Controller.prototype.cw = function (angle, callback) {
    ...
    this._go({x: state.x, y: state.y, z: state.z, yaw: yaw.toRad()}, callback);
};
...
Controller.prototype.up = function (distance, callback) {
    var state = this.state();
    this._go({x: state.x, y: state.y, z: state.z + distance, yaw: state.yaw}, callback);
};
...
Controller.prototype.altitude = function (altitude, callback) {
    var state = this.state();
    this._go({x: state.x, y: state.y, z: altitude, yaw: state.yaw}, callback);
};
...
Controller.prototype.go = function (goal, callback) {
    ...
    this._go(goal, callback);
};

Controller.prototype._go = function (goal, callback) {

    if(!this._security.check(goal)) {
        callback({
            message: 'Security box failure - leaving box.',
            goal: goal,
            securityBox: this._security.getSecurityBox()
        });
        return;
    }
